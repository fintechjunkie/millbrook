...
export const ARCS = [
  {
    ...
    start: {
      ...
      hint:
        ...
        + 'to, and what everybody in Millbrook already knows.',
    },
...
export const UPCOMING_ARCS = [
  {
    ...
    // Slate, the next tint along. Deliberately the cool one next to The Patch's warm
    // sand, so the two bands read as two places at a glance.
    band: 'slate',
    status: 'coming soon',
    // Real titles rather than a count, which is the whole change here: an arc that names
    // its four volumes is a promise, and four numbered blanks are an apology. `slug` is
    // absent on purpose — nothing is clickable until there is something to click into,
    // and a route that 404s would be worse than no route.
    volumes: [
      { vol: 1, title: 'Curfew' },
      { vol: 2, title: 'Four Out Of The Trunk' },
      { vol: 3, title: 'The Inventory' },
      { vol: 4, title: 'Declared Civil Contingency' },
    ],
  },
];
