 ...
 *
 * **Why `morphMs` exists.** The hand-off used to be abrupt for a reason no amount of
 * fading could fix: the clone showed the volume COVER and the page underneath showed the
 * chapter OPENER, so the cross-dissolve was between two different pictures and read as a
 * jump cut however slowly it ran. The clone now dissolves cover into opener mid-flight,
 * which means the final hand-off is between two copies of the same image and is therefore
 * invisible. It also makes the effect literally a transformation rather than a resize.
 *
 * Tune here. Nothing else hardcodes these.
 */
export const open = {
  liftMs: 280,   // cover rises off the shelf, everything else dims
  growMs: 780,   // grows to the reader's 2:1 and tips about its left edge
  morphMs: 520,  // the cover cross-dissolves into the chapter opener
  holdMs: 620,   // rests on the opener, flat and full size, before the route changes
  fadeMs: 420,   // clone dissolves into the identical page beneath it
};

// Total before navigation.
open.navMs = open.liftMs + open.growMs + open.morphMs + open.holdMs;
...
export const geometry = {
  ...
  readerPad: {
    inline: 24, top: 44, bottom: 64,
    compactInline: 10, compactTop: 38, compactBottom: 58,
  },
};
